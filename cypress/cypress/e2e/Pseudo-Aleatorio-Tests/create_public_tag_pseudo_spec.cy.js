describe("Public Tag Creation", () => {
  ...
  it("allows a user to create a public Tag with pseudoRandom data", async () => {
  ...
  it("allows a user to create a public Tag with pseudoRandom data and add a description with less than 500 characters (pseudoRandom)", async () => {
    const data = await getMockarooData();
    cy.get('a[class="ember-view gh-btn gh-btn-primary"]').click();
    cy.get('input[id="tag-name"]').type(data[0].tagName);
  ...
  it("allows a user to create a public Tag with pseudoRandom data and edit the slug with pseudoRandom data", async () => {
    const data = await getMockarooData();
    cy.get('a[class="ember-view gh-btn gh-btn-primary"]').click();
    cy.get('input[id="tag-name"]').type(data[0].tagName);
  ...
  it("wont allow an user to create a public Tag with pseudoRandom data and add a description with more than 500 characters (pseudoRandom)", async () => {
    const data = await getMockarooData();
    cy.get('a[class="ember-view gh-btn gh-btn-primary"]').click();
    cy.get('input[id="tag-name"]').type(data[0].tagName);
