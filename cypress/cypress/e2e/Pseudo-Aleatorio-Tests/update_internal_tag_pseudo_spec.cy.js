...
describe("Internal Tag Editing", () => {
  ...
  it("allows a user to update a internal Tag with pseudo random data", async () => {
  ...
  it("allows a user to update a internal Tag description with pseudo random data", async () => {
    const data = await getMockarooData();
    cy.contains("Internal tags").first().click();
    cy.get('a[title="Edit tag"]').first().click();
    cy.get('input[id="tag-name"]').clear();
  ...
  it("will not allows a user to update a internal Tag description with pseudo random data if its over 500 characters", async () => {
    const data = await getMockarooData();
    cy.contains("Internal tags").first().click();
    cy.get('a[title="Edit tag"]').first().click();
    cy.get('input[id="tag-name"]').clear();
