...
describe("Public Tag Editing", () => {
  ...
  it("allows a user to update a public Tag with random data", () => {
  ...
  it("allows a user to update a public Tag description with random data", () => {
    cy.get('a[title="Edit tag"]').first().click();
    cy.get('input[id="tag-name"]').clear();
    cy.get('input[id="tag-name"]').type(faker.lorem.words(2));
  ...
  it("will not allows a user to update a public Tag description with random data if its over 500 characters", () => {
    cy.get('a[title="Edit tag"]').first().click();
    cy.get('input[id="tag-name"]').clear();
    cy.get('input[id="tag-name"]').type(faker.lorem.words(2));
    cy.get('textarea[id="tag-description"]').clear();
